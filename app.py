...
sqlconnection.execute("create table if not exists calling(name text, rating number,message text)")
...
def contact():
     if request.method =="POST":
        try:
            ...
            crating=request.form['rating']
            cmessage=request.form['message']
            sqlconnection= sqlite3.connect('beauty.db')
       
            cur=sqlconnection.cursor()
            cur.execute("insert into calling(name,rating,message)values(?,?,?)",(cname,crating,cmessage))
            sqlconnection.commit()
            flash("Message has been sent","record")
        except:
             flash("Opps! Please Try Again","oops")
